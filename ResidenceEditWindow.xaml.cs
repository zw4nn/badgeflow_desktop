...
{
    public string ResidenceName=>NameBox.Text.Trim(); public string Manager=>ManagerBox.Text.Trim(); public string Address=>AddressBox.Text.Trim(); public string PostalCode=>PostalCodeBox.Text.Trim(); public string City=>CityBox.Text.Trim();
    ...
    public ResidenceEditWindow(Residence? residence=null,IEnumerable<string>? softwareSuggestions=null,IEnumerable<string>? managerSuggestions=null)
    {
        InitializeComponent(); SoftwareBox.ItemsSource=(softwareSuggestions??Array.Empty<string>()).OrderBy(x=>x).ToList(); ManagerBox.ItemsSource=(managerSuggestions??Array.Empty<string>()).OrderBy(x=>x).ToList();
        if(residence is not null){TitleText.Text="Modifier la résidence";NameBox.Text=residence.Name;ManagerBox.Text=residence.Manager;AddressBox.Text=residence.Address;PostalCodeBox.Text=residence.PostalCode;CityBox.Text=residence.City;UrmetBox.IsChecked=Has(residence,"Urmet");HexactBox.IsChecked=Has(residence,"Hexact");IntratoneBox.IsChecked=Has(residence,"Intratone");StarproxBox.IsChecked=Has(residence,"Starprox");OtherBox.IsChecked=Has(residence,"Autre");for(int i=0;i<ManagementBox.Items.Count;i++)if(((ComboBoxItem)ManagementBox.Items[i]).Content?.ToString()==residence.ManagementMode)ManagementBox.SelectedIndex=i;SoftwareBox.Text=residence.Software;ManagementNotesBox.Text=residence.ManagementNotes;}
