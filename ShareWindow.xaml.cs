...
public partial class ShareWindow : Window
{
    ...
    private readonly (string Key, string Title, bool ResidentLevel)[] _columns =
    {
        ...
        ("starprox","Starprox",false),("libelle","Libelle / reference",false),("datecreation","Date creation",false),("notes","Notes badge",false)
    };
    ...
    private void ExportCsv_Click(object sender, RoutedEventArgs e)
    {
        ...
        if ((residents.Count == 0 && _residence.DirectBadges.Count == 0) || cols.Count == 0) { MessageBox.Show("Aucune donnée à exporter ou aucune colonne sélectionnée."); return; }
    ...
    {
        var residents = SelectedResidents(); if (residents.Count == 0 && _residence.DirectBadges.Count == 0) { MessageBox.Show("Aucune donnée à partager."); return; }
        ...
        if(_residence.DirectBadges.Count>0){b.AppendLine();b.AppendLine("Badges sans résident :");foreach(var badge in _residence.DirectBadges){b.AppendLine();if(badge.Starprox)b.AppendLine("ATTENTION BADGE STARPROX");b.AppendLine($"Technologie : {badge.Technology}");b.AppendLine($"Numéro : {badge.Number}");if(!string.IsNullOrWhiteSpace(badge.Label))b.AppendLine($"Libellé : {badge.Label}");}}
    ...
    private string BuildCsv(List<Resident> residents, List<(string Key,string Title,bool ResidentLevel)> cols)
    {
        ...
        foreach(var badge in _residence.DirectBadges){b.AppendLine(string.Join(";", cols.Select(c => Csv(Ascii(ValueDirect(c.Key,badge))))));}
        return b.ToString();
    }
    ...
    private string Value(string key, Resident r, BadgeRecord? b) => key switch
    {
        ...
        "hex" => b?.Hex ?? "", "decimal" => b?.Decimal.ToString(CultureInfo.InvariantCulture) ?? "", "starprox" => b?.Starprox == true ? "OUI" : "", "libelle" => b?.Label ?? "", "datecreation" => b is null ? "" : b.CreatedAt.ToString("dd/MM/yyyy HH:mm"), "notes" => b?.Notes ?? "", _ => ""
    };
    private string ValueDirect(string key, BadgeRecord b) => key switch
    {
        "residence" => _residence.Name, "ville" => _residence.City, "technologie" => b.Technology, "numero" => b.Number, "hex" => b.Hex, "decimal" => b.Decimal.ToString(CultureInfo.InvariantCulture), "starprox" => b.Starprox ? "OUI" : "", "libelle" => b.Label, "datecreation" => b.CreatedAt.ToString("dd/MM/yyyy HH:mm"), "notes" => b.Notes, _ => ""
    };
