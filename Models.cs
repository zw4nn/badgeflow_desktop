...
public sealed class Residence
{
    ...
    public List<BadgeRecord> DirectBadges { get; set; } = new();
    public string ManagementSummary => string.Join(" · ", new[] { Manager, Technologies, ManagementMode, Software }.Where(x => !string.IsNullOrWhiteSpace(x)));
...
public sealed class AgencyGroup
{
    public string Name { get; set; } = "";
    public List<Residence> Residences { get; set; } = new();
    public string Header => $"{Name}  ·  {Residences.Count} résidence(s)";
}
